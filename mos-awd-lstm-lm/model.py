...
class RNNModel(nn.Module):
    ...
    def forward(self, input, hidden, return_h=False, return_prob=False, targets=None, is_switch=False, epoch=0):
        ...
        if not self.post_process:
            ...
            emb, sigma = embedded_dropout(self.encoder, self.encoder.weight, input, dropout=self.dropoute if (self.training and self.use_dropout) else 0, is_training=self.training, post_process=True)
        ...
        if epoch <= self.final_switch and self.training and self.use_dropout:
            
            logit = self.decoder(latent.view(-1, self.ninp))
            _latent = latent.view(-1, self.ninp)
            ...
            n_w = torch.sqrt(torch.sum(self.encoder(targets)**2, 1, keepdim=True) + 1e-8)
            cos_theta = (torch.sum(_latent * self.encoder(targets), 1, keepdim=True)) / n_output / n_w
